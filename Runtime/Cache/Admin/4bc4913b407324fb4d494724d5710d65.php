...
<head>
    <meta charset="UTF-8">
    <title><?php echo ($meta_title); ?>|MAS管理平台</title>
		...
		<div class="tab-content">
			...
			<form id="form" action="<?php echo U('update');?>" method="post" class="form-horizontal doc-modal-form">
				...
				<div id="tab2" class="tab-pane tab2">
					...
					<div class="form-item cf">
						<label class="item-label">表单显示分组<span class="check-tips">（用于表单显示的分组）</span></label>
						<div class="controls">
					...
					<?php $_result=parse_field_attr($info['field_group']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="form-item cf edit_sort edit_sort_l form_field_sort">
							...
							<ul class="dragsort" data-group="<?php echo ($key); ?>">
								<?php if(is_array($fields)): foreach($fields as $k=>$field): if((($field['group'] == $key) or (($i == 1) and ($field['group'] == 1))) and ($field['is_show'] == 1)): ?><li class="getSort">
				...
				<div id="tab3" class="tab-pane tab3">
					<div class="form-item cf">
						<label class="item-label">列表模板<span class="check-tips">（自定义的列表模板，不写则使用默认模板）</span></label>
						<div class="controls">
							<input type="text" class="text input-large" name="template_list" value="<?php echo ($info["template_list"]); ?>">
						</div>
					</div>
					<div class="form-item cf">
						<label class="item-label">新增模板<span class="check-tips">（自定义的新增模板，不写则使用默认模板）</span></label>
						<div class="controls">
							<input type="text" class="text input-large" name="template_add" value="<?php echo ($info["template_add"]); ?>">
						</div>
					</div>
					<div class="form-item cf">
						<label class="item-label">编辑模板<span class="check-tips">（自定义的编辑模板，不写则使用默认模板）</span></label>
						<div class="controls">
					...
					<div class="form-item cf">
						...
						<div class="controls">
							<input type="text" class="text input-large" name="list_row" value="<?php echo ($info["list_length"]); ?>">
						</div>
					</div>
				</div>
